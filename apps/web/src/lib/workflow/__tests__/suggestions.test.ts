...
describe('buildScoredUsers', () => {
	...
	it('builds a single user with a single assignment', () => {
		const assignments = [makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, priority: 2 })];
	...
	it('calculates positive availability score when user has a task', () => {
		const assignments = [makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, priority: 1 })];
...
describe('generateWorkflowSuggestions', () => {
	...
	it('resolves a single breach with one eligible user', () => {
		const loc = makeLocation({ deskLocationId: 10, deskLocationName: 'Loc A', openTaskUnits: 30, capacityThreshold: 20 });
		// severity = 10/20 = 0.5, eligible = 1, usersNeeded = ceil(0.5 * 1) = 1
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Loc A', priority: null }),
		];
	...
	it('resolves multiple breaches in severity order, state carries across', () => {
		const locHigh = makeLocation({ deskLocationId: 10, deskLocationName: 'High', openTaskUnits: 40, capacityThreshold: 20 }); // severity 1.0
		const locLow = makeLocation({ deskLocationId: 20, deskLocationName: 'Low', openTaskUnits: 22, capacityThreshold: 20 }); // severity 0.1

		// User eligible for both locations
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'High', priority: null }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'Low', priority: null }),
		];
	...
	it('reports partial resolution when not enough users', () => {
		...
		const loc = makeLocation({ deskLocationId: 10, deskLocationName: 'Loc A', openTaskUnits: 40, capacityThreshold: 20 });

		// Two eligible users, but one has all P1-P5 occupied by OTHER locations
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Loc A', priority: null }),
			makeAssignment({ userId: 'u2', userName: 'Bob', deskLocationId: 10, deskLocationName: 'Loc A', priority: null }),
			// Fill Bob's P1-P5 with other locations
			makeAssignment({ userId: 'u2', userName: 'Bob', deskLocationId: 101, deskLocationName: 'X1', priority: 1 }),
			makeAssignment({ userId: 'u2', userName: 'Bob', deskLocationId: 102, deskLocationName: 'X2', priority: 2 }),
			makeAssignment({ userId: 'u2', userName: 'Bob', deskLocationId: 103, deskLocationName: 'X3', priority: 3 }),
			makeAssignment({ userId: 'u2', userName: 'Bob', deskLocationId: 104, deskLocationName: 'X4', priority: 4 }),
			makeAssignment({ userId: 'u2', userName: 'Bob', deskLocationId: 105, deskLocationName: 'X5', priority: 5 }),
		];
	...
	it('cascades priorities when assigning P1 and P1 is already taken', () => {
		const loc = makeLocation({ deskLocationId: 10, deskLocationName: 'Target', openTaskUnits: 30, capacityThreshold: 20 });

		// User has P1 at location 20 and is eligible for location 10
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Target', priority: null }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'Other', priority: 1 }),
		];
	...
	it('cascades P1→P2→P3 when inserting at P1', () => {
		...
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Target', priority: 3 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'Other1', priority: 1 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 30, deskLocationName: 'Other2', priority: 2 }),
		];
	...
	it('skips users with all priority slots exhausted (P1-P5 occupied by other locations)', () => {
		...
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Target', priority: null }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'A', priority: 1 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 30, deskLocationName: 'B', priority: 2 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 40, deskLocationName: 'C', priority: 3 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 50, deskLocationName: 'D', priority: 4 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 60, deskLocationName: 'E', priority: 5 }),
		];
	...
	it('cascade past P5 produces cascadedChange with newPriority null', () => {
		...
		const locA = makeLocation({ deskLocationId: 10, deskLocationName: 'Location A', openTaskUnits: 30, capacityThreshold: 10 });
		...
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Location A', priority: null }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'Loc B', priority: 2 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 30, deskLocationName: 'Loc C', priority: 3 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 40, deskLocationName: 'Loc D', priority: 4 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 50, deskLocationName: 'Loc E', priority: 5 }),
		];
	...
	it('produces correct summary counts', () => {
		...
		const loc1 = makeLocation({ deskLocationId: 10, deskLocationName: 'Loc1', openTaskUnits: 25, capacityThreshold: 20 }); // severity 0.25
		const loc2 = makeLocation({ deskLocationId: 20, deskLocationName: 'Loc2', openTaskUnits: 60, capacityThreshold: 20 }); // severity 2.0 → capped 1.0

		// Two users, both eligible for loc1 and loc2
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Loc1', priority: null }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'Loc2', priority: null }),
			makeAssignment({ userId: 'u2', userName: 'Bob', deskLocationId: 10, deskLocationName: 'Loc1', priority: null }),
			makeAssignment({ userId: 'u2', userName: 'Bob', deskLocationId: 20, deskLocationName: 'Loc2', priority: null }),
		];
		...
		const totalFromResolutions = result.resolutions.reduce((sum, r) => sum + r.assignments.length, 0);
	...
	it('affectedUserAssignments only includes users whose state changed', () => {
		const loc = makeLocation({ deskLocationId: 10, deskLocationName: 'Loc A', openTaskUnits: 25, capacityThreshold: 20 });
		...
		expect(result.affectedUserAssignments[0].userId).toBe(result.resolutions[0].assignments[0].userId);
	});

	it('cascades priorities in ascending order across multiple locations', () => {
		// Breached location with high severity
		const loc = makeLocation({ deskLocationId: 10, deskLocationName: 'Breached', openTaskUnits: 30, capacityThreshold: 10 });
		...
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Breached', priority: null }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'Loc B', priority: 3 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 30, deskLocationName: 'Loc C', priority: 4 }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 40, deskLocationName: 'Loc D', priority: 5 }),
		];
		...
		expect(resolution.cascadedChanges[0]).toMatchObject({ deskLocationId: 20, previousPriority: 3, newPriority: 4 });
		expect(resolution.cascadedChanges[1]).toMatchObject({ deskLocationId: 30, previousPriority: 4, newPriority: 5 });
		expect(resolution.cascadedChanges[2]).toMatchObject({ deskLocationId: 40, previousPriority: 5, newPriority: null });
	});

	it('affectedUserAssignments includes null priority for locations without active priority', () => {
		// Breached location
		const loc = makeLocation({ deskLocationId: 10, deskLocationName: 'Breached', openTaskUnits: 30, capacityThreshold: 20 });
		...
		const assignments = [
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Breached', priority: null }),
			makeAssignment({ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'Other', priority: null }),
		];
...
describe('extractApprovedAssignments', () => {
	function makeSuggestionWithResolutions(): WorkflowSuggestion {
		return {
			...
			resolutions: [
				{
					...
					usersNeeded: 1,
					assignments: [{ userId: 'u1', userName: 'Alice', deskLocationId: 10, deskLocationName: 'Loc A', newPriority: 1, previousPriority: null }],
					cascadedChanges: [{ userId: 'u1', userName: 'Alice', deskLocationId: 20, deskLocationName: 'Loc B', previousPriority: 1, newPriority: 2 }],
					shortfall: 0,
					skippedUsers: [],
				},
				{
					...
					usersNeeded: 1,
					assignments: [{ userId: 'u2', userName: 'Bob', deskLocationId: 30, deskLocationName: 'Loc C', newPriority: 1, previousPriority: null }],
					cascadedChanges: [],
					shortfall: 0,
					skippedUsers: [],
				},
			...
			affectedUserAssignments: [
				{ userId: 'u1', assignments: [{ deskLocationId: 10, priority: 1 }, { deskLocationId: 20, priority: 2 }] },
