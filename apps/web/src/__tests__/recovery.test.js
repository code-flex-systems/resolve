...
async function main() {
  ...
  try {
    ...
    console.log(`✓ Created deadline #${deadline1.id}: ${deadline1.deadline_type} (${deadline1.deadline_date.toISOString().split('T')[0]})`);
    console.log(`✓ Created deadline #${deadline2.id}: ${deadline2.deadline_type} (${deadline2.deadline_date.toISOString().split('T')[0]})\n`);
    ...
    console.log(`  Deadlines: ${deadlines.length} (${deadlines.filter(d => d.status === 'met').length} met, ${deadlines.filter(d => d.status === 'pending').length} pending)\n`);
