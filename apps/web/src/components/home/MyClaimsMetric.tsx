...
export default function MyClaimsMetric() {
	...
	const totalOpen =
		(stats[ClaimStatus.IN_PROGRESS] ?? 0) +
		(stats[ClaimStatus.BLOCKED] ?? 0) +
		(stats[ClaimStatus.UNWORKED] ?? 0);
				...
				{isLoading ? (
					...
					>
						...
						<div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
							<span style={{ fontSize: 48, fontWeight: 700, color: 'primary' }}>{totalOpen}</span>
						...
						<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
							<Chip
								color="warning"
								size="sm"
							>{`${stats[ClaimStatus.IN_PROGRESS] ?? 0} In Progress`}</Chip>
