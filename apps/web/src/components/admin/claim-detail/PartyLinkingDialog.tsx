...
export default function PartyLinkingDialog({
	...
	useEffect(() => {
		if (editingClaimParty) {
			...
			if (editingClaimParty.parent_claim_party_id && availableParentEntities.length > 0) {
				const parent = availableParentEntities.find(
					(p) => p.id === editingClaimParty.parent_claim_party_id
				);
	...
	const handleSubmit = async () => {
		...
		await onSubmit({
			...
			// Free-form representative (entities)
			representative_name:
				!isFacilitatorMode && formData.representative_name ? formData.representative_name : null,
			// Other fields
			liability_percentage: formData.liability_percentage
				? parseFloat(formData.liability_percentage)
				: null,
			...
			policy_limit:
				isFacilitatorMode && formData.policy_limit ? parseFloat(formData.policy_limit) : null,
		});
	};
	...
	const newLiabilityValue = formData.liability_percentage
		? parseFloat(formData.liability_percentage)
		: 0;
	...
	const selectedParentEntityOption: ComboboxOption | null = selectedParentEntity
		? {
				...
				label: selectedParentEntity.party?.name || '',
				description: selectedParentEntity.role ?? undefined,
			}
		: null;
	...
	const partyComboboxOptions: ComboboxOption[] = partyAutocompleteOptions.map((p: any) => ({
		...
		description:
			p.id === -2
				? undefined
				: `${p.organization || 'No organization'} • ${p.address_city ? (p.address_state ? `${p.address_city}, ${p.address_state}` : p.address_city) : 'No address'}`,
		disabled: p.id === -2,
	}));
	...
	const repComboboxOptions: ComboboxOption[] = representativeAutocompleteOptions.map(
		(rep: any) => ({
			value: rep.id,
			label: `${rep.first_name} ${rep.last_name || ''}`.trim(),
			description:
				!selectedParty && rep.party_name
	...
	const selectedRepOption: ComboboxOption | null = selectedRepresentative
		? {
				...
				label:
					`${selectedRepresentative.first_name} ${selectedRepresentative.last_name || ''}`.trim(),
			}
		: null;
	...
	const step1Valid = creatingNewParty
		...
		: !!formData.party_id &&
			(!isFacilitatorMode || !!formData.parent_claim_party_id || !!parentClaimPartyId);
	const newPartyNameValid = newPartyData.is_business
		? !!newPartyData.name.trim()
		: !!(newPartyData.first_name.trim() && newPartyData.last_name.trim());
	const step2Valid =
		formData.role.length > 0 && isValidLiabilityPercentage && !wouldExceedTotalLiability;
							...
							{newPartyData.is_business ? (
								...
								<div style={{ display: 'flex', gap: 12 }}>
									<Input
										...
										onChange={(e) =>
											setNewPartyData({ ...newPartyData, first_name: e.target.value })
										}
										fullWidth
										placeholder="First"
									/>
									<Input
										...
										onChange={(e) =>
											setNewPartyData({ ...newPartyData, last_name: e.target.value })
										}
										fullWidth
										placeholder="Last"
									/>
					...
					content: (
						<div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
							...
							<Input
								...
								onChange={(e) =>
									setNewPartyData({ ...newPartyData, contact_email: e.target.value })
								}
								fullWidth
							...
							<Input
								...
								onChange={(e) =>
									setNewPartyData({ ...newPartyData, contact_phone: e.target.value })
								}
								fullWidth
				...
				<div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
					...
					{isFacilitatorMode && !parentClaimPartyId && (
						<Combobox
							...
							onChange={(opt) => {
								const entity = opt
									? parentEntityOptions.find((pe: any) => pe.id === opt.value)
									: null;
					...
					<Combobox
						...
						onChange={(opt) => {
							const party = opt
								? partyAutocompleteOptions.find((p: any) => p.id === opt.value)
								: null;
				...
				<div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
					...
					<Combobox
						...
						onChangeMultiple={(opts) =>
							setFormData({ ...formData, role: opts.map((o) => String(o.value)) })
						}
						isOptionEqual={(a, b) => a.value === b.value}
						label="Roles *"
						...
						<Input
							...
							errorText={
								!isValidLiabilityPercentage
									...
									: wouldExceedTotalLiability
										? `Combined liability cannot exceed 100% (currently ${currentTotalLiability.toFixed(1)}% allocated)`
										: undefined
							}
							helperText={
								isValidLiabilityPercentage && !wouldExceedTotalLiability
									? currentTotalLiability > 0
										...
										: "This party's percentage of liability"
									: undefined
							}
						/>
					)}
					{isFacilitatorMode && (
						<LossTypeSelect
							...
							setLossType={(lt) => setFormData({ ...formData, loss_type: lt })}
							clearable
							isFilter={false}
							label="Loss Type"
						/>
						...
						<>
							<div>
								<Combobox
									...
									onChange={(opt) => {
										const addr = opt
											? (addresses as any[]).find((a: any) => a.id === opt.value)
											: null;
							...
							<div>
								<Combobox
									...
									onChange={(opt) => {
										...
										const rep = representativeAutocompleteOptions.find(
											(r: any) => r.id === opt.value
										);
										if (rep?._isHint) return;
										handleRepresentativeSelect(rep);
									}}
									onInputChange={(value) => {
										if (!selectedParty) setRepSearchTerm(value);
									}}
									isOptionEqual={(a, b) => a.value === b.value}
									filterDisabled
									label="Representative *"
						...
						placeholder={
							isFacilitatorMode
								...
								: roleListEntity === 'claimant_party_role'
									...
									: "Notes about this entity's liability..."
						}
					/>
				</div>
			),
			...
			content: (
				<div style={{ display: 'flex', flexDirection: 'column' as const, gap: 12 }}>
					...
					<div>
						...
						>
							{isFacilitatorMode ? 'Facilitator' : 'Entity'}
						</span>
					...
					<div>
						...
						<div style={{ display: 'flex', gap: 4, marginTop: 4, flexWrap: 'wrap' as const }}>
							{selectedRoleOptions.map((r) => (
								<Chip key={String(r.value)} size="sm" color="info">
									{r.label}
								</Chip>
							))}
	...
	const allSteps: Step[] = [dynamicSteps[0], ...partyCreationSteps, ...dynamicSteps.slice(1)];
	...
	return (
		<Dialog open={open} onClose={onClose} size="lg">
			<StepperFlow
				...
				onCancel={onClose}
				completeLabel={
					creatingNewParty ? 'Create & Continue' : editingClaimParty ? 'Update' : 'Add'
				}
			...
			{showCreateAddressDialog && selectedParty && (
				<AddressDialog
					address={{ party_id: selectedParty.id, party_name: selectedParty.name } as any}
