...
export async function createChecklist(claimId: number, params: object) {
...
export async function getChecklist(checklistId: number) {
	return await db.selectFrom('checklist').selectAll().where('id', '=', checklistId).executeTakeFirstOrThrow();
}

export async function getChecklists(searchTerm?: string) {
	let query = db
...
export async function getChecklistClaim(checklistId: number, claimId: number) {
	return await db
...
export async function getChecklistSummary(checklistId: number, claimId: number) {
	return await db
...
export async function getChecklistSummaryDetail({
	...
	switch (segment) {
		case SummarySegment.ANSWERED:
			query = query.where((qb) => qb.or([qb('qr.response_text', 'is not', null), qb('qra.id', 'is not', null)]));
			break;
	...
	if (mode === 'count') {
		const result = await query.select(sql<number>`count(distinct q.id)`.as('count')).executeTakeFirstOrThrow();
...
export async function modifyChecklist(checklistId: number, params: object) {
	return await db
