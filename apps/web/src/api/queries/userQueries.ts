...
export async function upsertUserFromClerk(
	...
	// email_verified is a timestamp column - set to now() if verified, null if not
	const emailVerifiedAt = user.email_verified ? new Date() : null;

	// Build the update set object, only including email_verified if user is verified
	// This prevents clearing an existing verification timestamp
	...
	if (user.email_verified) {
		// Use sql to conditionally set: if already verified keep it, otherwise set to now
		updateSet.email_verified = sql`COALESCE(users.email_verified, now())`;
	}
