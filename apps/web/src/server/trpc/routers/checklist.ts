...
export const checklistRouter = router({
	...
	getChecklistCount: protectedProcedure
		.input(getChecklistCountInput)
		.query(async ({ input, ctx }) => {
			...
			return await getChecklistCount(ctx, {
	...
	getChecklistClaim: protectedProcedure
		.input(getChecklistClaimInput)
		.query(async ({ input, ctx }) => {
	...
	getChecklistClaimStats: protectedProcedure
		.input(getChecklistClaimStatsInput)
		.query(async ({ ctx, input }) => {
			if (
				!!input.checklistId ||
				input.users?.length !== 1 ||
				input.users[0] !== ctx.session.user.id
			) {
				requireRole(ctx, [config.ROLES.ADMIN, config.ROLES.SUPER_ADMIN]);
			}
	...
	getChecklistClaims: protectedProcedure
		.input(getChecklistClaimsInput)
		.query(async ({ input, ctx }) => {
			requireRole(ctx, [config.ROLES.ADMIN, config.ROLES.SUPER_ADMIN]);
			return await getChecklistClaims(ctx, input);
		}),

	exportChecklistClaims: protectedProcedure
		.input(exportChecklistClaimsInput)
		.query(async ({ input, ctx }) => {
			requireRole(ctx, [config.ROLES.ADMIN, config.ROLES.SUPER_ADMIN]);
	...
	getChecklistSummary: protectedProcedure
		.input(getChecklistSummaryInput)
		.query(async ({ input, ctx }) => {
	...
	updateChecklist: protectedProcedure
		.input(modifyChecklistInput)
		.mutation(async ({ input, ctx }) => {
			requireRole(ctx, [config.ROLES.ADMIN, config.ROLES.SUPER_ADMIN]);
			return modifyChecklist(ctx, input);
		}),

	updateChecklistClaim: protectedProcedure
		.input(modifyChecklistClaimInput)
		.mutation(async ({ input, ctx }) => {
			if (!checkRole(ctx, [config.ROLES.ADMIN, config.ROLES.SUPER_ADMIN])) {
