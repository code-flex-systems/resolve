...
export const responseRouter = router({
	evaluateResponses: protectedProcedure
		.input(evaluateResponsesInput)
	...
	getResponsesForAnswer: protectedProcedure
		.input(getResponsesForAnswerInput)
	...
	getResponseAuditLogs: protectedProcedure
		.input(getResponseAuditLogsInput)
		.query(async ({ input, ctx }) => {
	...
	exportResponseAuditLogs: protectedProcedure
		.input(exportResponseAuditLogsInput)
		.query(async ({ input, ctx }) => {
	...
	getResponseAuditLogStats: protectedProcedure
		.input(getResponseAuditLogStatsInput)
		.query(async ({ input, ctx }) => {
			requireRole(ctx, [config.ROLES.ADMIN, config.ROLES.SUPER_ADMIN]);
	...
	getChecklistActivity: protectedProcedure
		.input(getResponseAuditLogStatsInput)
		.query(async ({ input, ctx }) => {
			requireRole(ctx, [config.ROLES.ADMIN, config.ROLES.SUPER_ADMIN]);
	...
	upsertQuestionResponses: protectedProcedure
		.input(upsertQuestionResponsesInput)
		.mutation(async ({ input, ctx }) => {
			const sampleResponse = input.responses?.[0];
			if (!sampleResponse)
				throw new TRPCError({ code: 'BAD_REQUEST', message: 'At least one response is required' });
			if (!checkRole(ctx, [config.ROLES.ADMIN, config.ROLES.SUPER_ADMIN])) {
